...
@MainActor
final class DynamicActionHandler {
    ...
    private let requestExecutor: DynamicRequestExecutor
    private let templateResolver: DynamicTemplateResolver

    init(dataStore: DynamicDataStore, requestExecutor: DynamicRequestExecutor) {
        self.dataStore = dataStore
        self.requestExecutor = requestExecutor
        self.templateResolver = DynamicTemplateResolver(dataStore: dataStore)
    }

    func handle(_ action: DynamicAction, resolver: DynamicTemplateResolver? = nil) async {
    ...
    private func openURL(_ rawURL: String) {
        guard
            ...
            DynamicPageLogger.debug("invalid url: \(rawURL)")
            return
        }
