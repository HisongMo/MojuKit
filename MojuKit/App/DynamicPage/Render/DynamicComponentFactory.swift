...
private final class DynamicFitContentContainerView: UIView {
...
@MainActor
final class DynamicComponentFactory {
    private let placeholderColor = UIColor.systemGray5

    func makeView(from component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView? {
        ...
        switch DynamicComponentType(rawValue: component.type) {
        case .text:
        ...
        case nil:
            DynamicPageLogger.debug("unsupported component: \(component.type)")
            #if DEBUG
        ...
        let skipsTapAction = contentView is UIButton || DynamicComponentType(rawValue: component.type) == .selectableCard
        attachTapActionIfNeeded(component.action, to: wrappedView, context: context, skipsButton: skipsTapAction)
        return wrappedView
    }

    private func makeText(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
        let label = UILabel()
    ...
    private func makeImage(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
        ...
        let task = Task { [weak imageView] in
            do {
                ...
                guard !Task.isCancelled, data.count <= DynamicPageLimits.maxResponseBytes else { return }
                ...
                DynamicPageLogger.debug("image load failed: \(url.absoluteString)")
            }
        }
    ...
    private func makeInput(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
    ...
    private func makeTextArea(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
        ...
        let delegate = DynamicTextViewDelegate(
    ...
    private func makeButton(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
    ...
    private func makeCard(_ component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView {
    ...
    private func makeRow(_ component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView {
        let stackView = UIStackView()
    ...
    private func makeTableView(_ component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView {
        let stackView = UIStackView()
        stackView.axis = .vertical
        stackView.spacing = component.style?.spacing ?? 0
        stackView.alignment = context.styleParser.stackAlignment(from: component.style?.stackAlignment)
    ...
    private func makeCollectionView(_ component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView {
        let stackView = UIStackView()
        stackView.axis = .vertical
        stackView.spacing = component.style?.spacing ?? 8
    ...
    private func renderListItems(_ component: DynamicComponent, into stackView: UIStackView, context: DynamicRenderContext, depth: Int) {
        let items = context.templateResolver.resolveArray(component.forEach)
        let itemName = component.forItem?.isEmpty == false ? component.forItem! : "item"
        let indexName = component.forIndex?.isEmpty == false ? component.forIndex! : "index"
    ...
    private func makeIcon(_ component: DynamicComponent, context: DynamicRenderContext) -> UIView {
        let imageView = UIImageView()
        let iconName = component.iconName.map { context.templateResolver.resolveString($0) } ?? ""
        imageView.image = UIImage(systemName: iconName) ?? UIImage(named: iconName)
    ...
    private func makeSelectableCard(_ component: DynamicComponent, context: DynamicRenderContext, depth: Int) -> UIView {
        ...
        tapRecognizer.addAction {
            Task { @MainActor in
                if let stateKey = component.stateKey, let value = component.value {
                    await context.actionHandler.handle(DynamicAction(type: "setState", stateKey: stateKey, value: value), resolver: context.templateResolver)
                }
                if let action {
    ...
    private func wrap(_ contentView: UIView, style: DynamicStyle?, parser: DynamicStyleParser, isContainer: Bool = false) -> UIView {
        let usesFitContentWidth = isFitContentWidth(style)
        let container: UIView = usesFitContentWidth ? DynamicFitContentContainerView() : UIView()
        ...
        if let fitContentContainer = container as? DynamicFitContentContainerView {
    ...
    private func isFitContentWidth(_ style: DynamicStyle?) -> Bool {
    ...
    private func shouldAddTrailingSpacer(to component: DynamicComponent) -> Bool {
    ...
    private func isVisible(_ component: DynamicComponent, context: DynamicRenderContext) -> Bool {
    ...
    private func effectiveStyle(for component: DynamicComponent, context: DynamicRenderContext) -> DynamicStyle? {
    ...
    private func isSelected(_ component: DynamicComponent, context: DynamicRenderContext) -> Bool {
        guard let stateKey = component.stateKey, let value = component.value else {
            return false
        }

        let current = DynamicValue.fromAny(context.dataStore.value(forKeyPath: stateKey))
        let expected = context.templateResolver.resolveValue(value)
        return current.stringValue == expected.stringValue
    }

    private func initialInputText(for component: DynamicComponent, context: DynamicRenderContext) -> String {
        if let stateKey = component.stateKey,
           let current = context.dataStore.value(forKeyPath: stateKey) {
            return DynamicValue.fromAny(current).stringValue ?? ""
        }
    ...
    private func syncInputValue(_ text: String, component: DynamicComponent, context: DynamicRenderContext) {
...
private final class DynamicTextViewDelegate: NSObject, UITextViewDelegate {
...
private extension UITapGestureRecognizer {
    func addAction(_ action: @escaping () -> Void) {
        let sleeve = DynamicGestureSleeve(action)
        addTarget(sleeve, action: #selector(DynamicGestureSleeve.invoke))
...
private final class DynamicGestureSleeve {
