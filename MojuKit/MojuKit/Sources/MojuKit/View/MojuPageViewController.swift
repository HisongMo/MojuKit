...
public final class MojuPageViewController: UIViewController {
    public var onNavigate: ((_ target: String, _ params: [String: MojuValue]?) -> Void)? {
        didSet { actionHandler?.onNavigate = onNavigate }
    }
    public var onNativeNavigate: ((_ target: String, _ params: [String: MojuValue]?) -> Void)? {
        didSet { actionHandler?.onNativeNavigate = onNativeNavigate }
    }
    public var onTrackEvent: ((_ eventName: String, _ params: [String: MojuValue]?) -> Void)? {
        didSet { actionHandler?.onTrackEvent = onTrackEvent }
    }
    public var onShowModal: ((_ target: String, _ params: [String: MojuValue]?) -> Void)? {
        didSet { actionHandler?.onShowModal = onShowModal }
    }
    public var onConfirmHighRiskRequest: ((_ apiKey: String, _ params: [String: Any], _ completion: @escaping (Bool) -> Void) -> Void)? {
    ...
    private var dataStore: MojuDataStore!
    private var requestExecutor: MojuRequestExecutor!
    private var actionHandler: MojuActionHandler!
    private var renderer: MojuPageRenderer!
    private var fixedBottomRenderer: MojuPageRenderer?
    private var loadTask: Task<Void, Never>?
    private var hasLoadedRequests = false

    public init(
        page: MojuPage,
        networkProvider: MojuNetworkProviding,
        imageProvider: MojuImageProviding = DefaultMojuImageProvider()
    ) {
        self.page = page
    ...
    private func setupDependencies() {
        dataStore = MojuDataStore(pageParams: page.pageParams)
        requestExecutor = MojuRequestExecutor(networkProvider: networkProvider, dataStore: dataStore)
        actionHandler = MojuActionHandler(dataStore: dataStore, requestExecutor: requestExecutor)
        renderer = MojuPageRenderer(
            page: page,
            stackView: stackView,
            dataStore: dataStore,
    ...
    private func setupViews() {
        ...
        view.backgroundColor = MojuStyleParser().color(from: page.backgroundColor, default: .systemBackground)
    ...
    private func renderInitialPage() {
        do {
            try MojuSchemaValidator.validate(page)
            fixedBottomRenderer = MojuPageRenderer(
                page: page,
        ...
        loadTask = Task { [weak self] in
            ...
            for request in requests {
                ...
                do {
                    ...
                    MojuPageLogger.debug("onLoad failed: \(request.apiKey)")
                }
            }
        }
    }
    ...
    private func showToast(_ message: String) {
        ...
        iconView.image = imageProvider.systemImage(named: iconName)
        ...
        if let config = page.navigationBar {
            ...
            if config.hideBackButton == true {
                ...
                if let iconName = customBack.iconName {
                    let image = imageProvider.systemImage(named: iconName) ?? imageProvider.image(named: iconName)
